...
class Rectangle {
  ...
  void makeRectangle(glm::mat4 VP, float x = 0.0f, float y = 0.0f, float angle = 0.0f, float rotX = 0, float rotY = 0, float scaling = 1.0f) {
    glm::mat4 MVP;
    Matrices.model = glm::mat4(1.0f);
    float rotAngle = (float) angle * (M_PI/180.0f);

    glm::mat4 scalingMatrix = glm::scale(glm::vec3(scaling, scaling ,scaling));
    ...
    Matrices.model *= (translateRectangle_2 * translateRectangle * rotateRectangle * translateRectangle_1 * scalingMatrix  );// * scaleTriangle);
